...
from .models import Contratante, Contrato, NotaContrato, NotaObra, Obra
...
class ObraForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "data_inicio_atividade": DateInput(
                attrs={"type": "date", "class": "form-control"}
            ),
            "data_termino_previsto": DateInput(
                attrs={"type": "date", "class": "form-control"}
            ),
            "porcentagem_execucao": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            if getattr(user, "is_superuser", False):
                ...
                qs = (
                    profile.contratos.filter(ativo=True)
                    if profile
        ...
        self.fields["contrato"].label_from_instance = (
            lambda obj: f"{obj.numero} - {obj.contratante}"
        )
        ...
        if (
            ...
            and getattr(self.instance, "contrato", None)
        ):
