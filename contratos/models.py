...
class Contratante(models.Model):
    nome = models.CharField(
        max_length=200,
        verbose_name="Empresa",
        help_text="Nome do contratante, ex: Empresa X",
    )
    ...
    ativo = models.BooleanField(
        default=True,
        help_text="Indica que o contratante será tratado como ativo. Ao invés de exclui-lo, desmarque isso.",
    )
...
class Contrato(models.Model):
    ...
    data_inicio = models.DateField(
        blank=True, null=True, help_text="Data de início da vigência do contrato"
    )
    data_fim = models.DateField(
        blank=True, null=True, help_text="Data de encerramento da vigência do contrato"
    )
...
class Obra(models.Model):
    codigo = models.CharField(
        max_length=50,
        unique=True,
        verbose_name="Cod/Nome Obra",
        help_text="Código da obra/serviço",
    )
    contrato = models.ForeignKey(
        Contrato, on_delete=models.CASCADE, related_name="obras"
    )
    ...
    descricao = models.TextField(
        blank=True, null=True, help_text="Descrição ou observações sobre a obra"
    )
    ativo = models.BooleanField(
        default=True,
        help_text="Indica que a obra será tratada como ativa. Ao invés de exclui-la, desmarque isso.",
    )
...
class NotaContrato(models.Model):
    contrato = models.ForeignKey(
        Contrato, on_delete=models.CASCADE, related_name="notas"
    )
