...
class PropostaOrcamentoAdmin(admin.ModelAdmin):
    list_display = (
        'titulo', 'contratante', 'local_execucao', 'valor_estimado',
        'data_envio', 'etapa_atual', 'situacao'
    )
    ...
    fieldsets = (
        ('Informações Gerais', {
            'fields': (
                'titulo', 'contratante', 'local_execucao', 'valor_estimado',
                'descricao', 'endereco_servidor'
            )
        }),
        ('Datas', {
            'fields': (
                'data_recebimento', 'data_envio', 'data_resposta'
            )
        }),
        ('Status e Controle', {
            'fields': (
                'etapa_atual', 'situacao'
            )
        }),
        ('Auditoria', {
            'classes': ('collapse',),
