...
class AtaReuniao(models.Model):
    ...
    contrato = models.ForeignKey(
        Contrato, on_delete=models.CASCADE, related_name="atas"
    )
...
class ItemAta(models.Model):
    ...
    categoria = models.ForeignKey(
        CategoriaItemAta, on_delete=models.SET_NULL, null=True
    )
    descricao = models.TextField()
    responsavel = models.CharField(
        max_length=100, null=True, blank=True, help_text="Responsável pela ação"
    )
    solicitante = models.CharField(
        max_length=100, null=True, blank=True, help_text="Quem solicitou essa ação"
    )
...
class ArquivoAta(models.Model):
    ata = models.ForeignKey(
        AtaReuniao, on_delete=models.CASCADE, related_name="arquivos"
    )
    ...
    id_pasta_drive = models.CharField(
        "ID da Pasta no Drive", max_length=100, blank=True, null=True
    )
